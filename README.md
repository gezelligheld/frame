...
- Fiber：Fiber作为一种数据结构表示了React组件树中的各个节点，每个Fiber节点包含了组件的状态和构建组件的其他信息，它们相互关联，形成一颗Fiber树

- 任务调度：concurrent 模式在进入调和流程之前先进行调度，调度器使用优先级确定任务的重要性和执行顺序，优先处理高优先级任务（这里的任务其实就是某个更新周期的 render 阶段），而 legacy 模式会按顺序执行更新任务，没有优先级之分

- 时间分片：时间分片会将大的渲染任务拆分成小的任务单元，避免long task造成页面长时间卡顿，借助 MessageChannel 在浏览器空闲时执行任务

- 可中断渲染：每个时间片段任务执行完毕后，会检查是否有更高优先级的任务以便于让出主线程。如果存在，会中断当前的任务并记录中断位置，将Fiber树回滚到更新前的状态，去执行高优先级任务，执行完毕后从中断位置开始继续执行
